...
export default function DashboardLogin({ slug, label }: { slug: string; label: string }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    setError("");
    const res = await fetch(`/api/events/${slug}/dashboard/login`, {
      method: "POST",
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4">
      ...
      >
        <h1 className="text-xl">{label} — dashboard login</h1>
        <input
          type="password"
          value={password}
